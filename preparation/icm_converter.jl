...
#icm_input = load_circuit_from_cirq_json("input_cirq_circuit.json")

"""
# icm_circuit is saved as as Vector{Tuple{String,Vector{String}}}() but Jabalizer.compile needs Vector{Tuple{String,Vector{Int64}}}()
# we convert the type here
new_icm_input = Vector{Tuple{String,Vector{Int64}}}()
for k in 1:length(icm_input)
    g = icm_input[k][1]
    q = icm_input[k][2]
    qn = Vector{Int64}()
    for i in 1:length(q)
    ...
    push!(new_icm_input, (g, qn))
end 
"""

#(icm_circuit,_) = Jabalizer.compile(icm_input, 10, gates_to_decomp, universal=false)

#
# based on Jabalizer.save_circuit_to_cirq with a fix for the qubit names & order
#
...
function save_circuit_to_cirq_json(circuit, file_name::String)
    ...
    for (gate, qubits) in circuit
        ...
        for qubit in qubits
            ...
            name = "9999999" * string(qubit)
            #print(name)
            #print("\n")
            cirq_qubit["name"] =  name #the 0 is to make sure these come before the flags
...
#save_circuit_to_cirq_json(icm_circuit,"test_out.json");
...
function icm_converter_ij(i, j)
    gates_to_decomp = ["T", "T^-1"];
    icm_input = load_circuit_from_cirq_json("input_cirq_circuit" * string(i) * string(j) * ".json")
    (icm_circuit,_) = Jabalizer.compile(icm_input, 10, gates_to_decomp, universal=false)
    save_circuit_to_cirq_json(icm_circuit,"output_cirq_icm_circuit" * string(i) * string(j) * ".json");
end

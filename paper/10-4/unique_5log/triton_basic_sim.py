...
if __name__ == '__main__':
    ...
    circuit_types = ["adder"]
    chosen_flags = -5
    circuits_folder = "precompiled_circuits/unique_5log"
    ...
    def run_simulation(icm_circuit, flag_circuit, error_mod, error_rate):
        results_flag, results_icm, acceptance, missed_flags, no_flags, false_flags, correct_flags = \
        ...
        return results_flag, results_icm, acceptance, missed_flags, no_flags, false_flags, correct_flags
    ...
    def parallel_simulation(inp, n_of_circuit_samples=n_of_circuit_samples):
        ...
        last_values = {
            ...
            "missed_flags": np.zeros(len(error_rates)),
            "no_flags": np.zeros(len(error_rates)),
            "false_flags": np.zeros(len(error_rates)),
            "correct_flags": np.zeros(len(error_rates))
        }
        ...
        for e in range(len(error_rates)):
            ...
            error_mod = 1.0
            large_fc_failure_rate, large_icm_failure_rate, acceptance, missed_flags, no_flags, false_flags, correct_flags = run_simulation(large_icm, large_fc, error_mod, error_rate)
            ...
            last_values["acceptance"][e] = acceptance[0]
            last_values["missed_flags"][e] = missed_flags[0]
            last_values["no_flags"][e] = no_flags[0]
            last_values["false_flags"][e] = false_flags[0]
            last_values["correct_flags"][e] = correct_flags[0]

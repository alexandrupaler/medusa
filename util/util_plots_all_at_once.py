...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 5:
        ...
        circuits_path = "precomputed_unique/{qubit_setting}/circuits" #f"{folder_path}/circuits"
    ...
    for filename in files:
        ...
        circuit_type_colors = ['blue', 'green', 'orange']
        # Check the circuit type
        for type_i in range(1): #len(chosen_circuit_types)):
            
            circuit_type_color = circuit_type_colors[type_i]
            chosen_circuit_type = chosen_circuit_types[type_i]

            #print(chosen_circuit_type)
            #print(circuit_type_color)
            ...
            if chosen_circuit_type in str(circuit_type):
                ...
                ax1.scatter(circuit_size_array, icm_small, color=circuit_type_color)
                ax1.set_ylabel("logical error rate")
                ax1.set_xlabel("circuit size")


                # difference between flag and icm error
                ax2.scatter(circuit_size_array, (np.array(res) - np.array(icm_small)), color=circuit_type_color)
                ...
                label_stuff = ['1e-4', '5e-4', '7e-4', '10e-4']
                ...
                ax6.legend(handles=[p1, p4]) #ax6.legend(handles=[p1, p2, p3, p4])
    ...
    #fig1.savefig(f"paper_plots_{plot_file_name_base}.png")
    #fig2.savefig(f"{plot_file_name_base}_diff.png")
    fig3.savefig(f"paper_plots_{plot_file_name_base}_mod.png")
    #fig4.savefig(f"paper_plots_{plot_file_name_base}_qubits.png")
    #fig5.savefig(f"paper_plots_{plot_file_name_base}_surface_mod.png")
    fig6.savefig(f"paper_plots_{plot_file_name_base}_surface_size.png")

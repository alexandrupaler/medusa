...
if __name__ == '__main__':
    ...
    for file in files:
        ...
        moments = flag_circuit.moments
        print("moments: ", len(moments))
        dqs = flag_circuit.all_qubits()
        fqs = list(filter(lambda q: 'f' in q.name, flag_circuit.all_qubits()))
        xqs = list(filter(lambda q: 'x' in q.name, fqs))
        zqs = list(filter(lambda q: 'z' in q.name, fqs))
        print(len(xqs), len(zqs))
    ...
    #plt.savefig("qubit_counts_unique_all.png")
...
def save_circuits(name, icm_circuit, flag_circuit):
    icm_json = cirq.to_json(icm_circuit)
    with open(f"circuits/icm_{name}.json", "w") as outfile:
        outfile.write(icm_json)

    # circuits/fc_name_i_j.json
    fc_json = cirq.to_json(flag_circuit)
    with open(f"circuits/fc_{name}.json", "w") as outfile:
        outfile.write(fc_json)


def generate_circuits(min_qubits, max_qubits, number_of_bench_samples):
